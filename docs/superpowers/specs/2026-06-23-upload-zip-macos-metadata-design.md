...
- top-level 에 디렉터리 하나만 있고 파일이 없는 동안 **반복**(최대 ~10단계 가드) →
  `a/b/index.html` 같은 다중 래핑도 처리. top-level 에 파일(html 등)이 있으면 평탄화하지
  않으므로 정상 구조는 절대 건드리지 않는다.
- 충돌 안전 절차: 단일 inner 디렉터리를 `<name>__lift` 로 rename → stage 가 비워짐 →
  inner 의 자식들을 빈 stage 로 rename → `__lift` rmdir. (파일명 충돌 불가)
