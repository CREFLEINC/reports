...
Basic Auth는 **서버 세션이 없다.** 브라우저가 자격증명을 캐시해 매 요청마다 자동 전송하므로,
서버가 무효화할 세션이 존재하지 않는다. 따라서 "긴급 요청"인 **로그아웃 기능**을 깔끔하게
구현하려면 인증 방식을 **세션/쿠키 기반**으로 전환해야 한다.

## 2. 목표 / 비목표

**목표**
- 사람 사용자에게 **확실히 동작하는 로그아웃**을 제공한다(쿠키 세션).
...
| 항목 | 결정 |
|------|------|
| 인증 방식 | 쿠키 세션 로그인/로그아웃 추가 (정공법) |
| 기존 Basic Auth | **하이브리드 유지** — 명시적 `Authorization` 헤더 클라이언트(자동화/CLI)용 폴백 |
| 역할 | 기존 2단계(`reader`/`uploader`) 유지 |
| 세션 저장 | Starlette `SessionMiddleware`(서명 쿠키, stateless) |
...
요청은 **둘 중 하나면** 인증 통과:
1. 유효한 **서명 세션 쿠키**(role: `reader` 또는 `uploader`), 또는
2. 유효한 **`Authorization: Basic` 헤더**(reader/uploader 자격증명 매칭) — 자동화 폴백.

의존성 동작:
- `verify`(읽기): 세션 role ∈ {reader, uploader} **또는** Basic이 reader/uploader 매칭 → 통과.
- `require_uploader`(쓰기): 세션 role == uploader **또는** Basic이 uploader 매칭 → 통과.
  `UPLOAD_PASS` 미설정 시 **503 fail-closed** 유지.
- 자격증명 비교는 `secrets.compare_digest`(상수시간) 유지. uploader를 먼저 검사해
...
- 브라우저(`Accept`에 `text/html` 포함) → `303 /login?next=<원경로>` 리다이렉트.
  **`WWW-Authenticate: Basic` 헤더를 더는 보내지 않는다** → 브라우저가 Basic 다이얼로그를
  띄우지 않고 Basic 자격증명을 캐시하지 않음 → **세션 쿠키 삭제(로그아웃)가 확실히 동작**.
- 비-브라우저(자동화/API) → `401 JSON`. `register_report.sh`는 올바른 `-u`를 보내므로 200.

구현 메모: 의존성에 `request: Request`를 주입해 `Accept`/경로를 읽는다. 브라우저 미인증 시
...
| `POST /login` | 공개 | username/password 검증(uploader 먼저). 성공 시 `session["user"]`,`session["role"]` 설정 후 `303 next`. `next`는 동일출처 상대경로(`/`로 시작, `//`·스킴 불허)만 허용 → 오픈리다이렉트 차단. 실패 시 폼에 오류(401 본문) |
| `POST /logout` | — | `request.session.clear()` 후 `303 /login?loggedout=1` |
...
- `SessionMiddleware`(서명 쿠키, stateless) — 서버 저장소 불필요, 멀티워커·재시작 무관.
- 신규 환경변수:
  - `REPORTS_SECRET_KEY` — 세션 서명 키. 미설정 시 기동 시 임시 랜덤(`secrets.token_hex(32)`)
    생성 + 경고 로그(기존 `REPORTS_PASS` 경고 패턴과 동일). 운영 영속성 위해 설정 권장.
  - `REPORTS_SESSION_MAX_AGE` — 세션 수명 초. 기본 `1209600`(14일).
  - `REPORTS_COOKIE_SECURE` — 기본 `0`(:28080 평문 HTTP). TLS 뒤에선 `1`.
- 쿠키 플래그: `HttpOnly`(기본), `SameSite=Lax`(`/login`·`/logout` POST CSRF 완화),
  `https_only=REPORTS_COOKIE_SECURE`.
- `requirements.txt`에 `itsdangerous>=2.0` 추가(SessionMiddleware 의존성).
...
- `docker-compose.yml`: 신규 env 3개(`REPORTS_SECRET_KEY`/`REPORTS_SESSION_MAX_AGE`/
  `REPORTS_COOKIE_SECURE`) 전달, 이미지 태그 `reporter:1.2 → 1.3`. 렌더러 이미지 불변.
- `.env.example`: 신규 env 추가(SECRET_KEY 강한 값 안내).
- `README.md` + `server.py` docstring: 로그인/로그아웃 흐름, 신규 env, "Basic은 자동화용으로
  유지" 명시.
...
- `POST /login`(reader) → 쿠키 설정, 이후 `/` → 200.
- `POST /login` 오답 → 쿠키 없음, 오류.
- reader 세션의 `GET /upload` → 거부(403/redirect), uploader 세션 → 200.
