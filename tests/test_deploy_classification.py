"""ops/deploy.sh 의 재빌드 판정이 Dockerfile 의 COPY 목록과 어긋나지 않도록 강제한다.

이른바 'COPY 트랩': 이미지에 들어가는 소스가 늘었는데 deploy.sh 의 재빌드 패턴에 추가하지
않으면, 그 파일만 바뀐 커밋은 재빌드 없이 마커(.deployed_sha)만 전진한다. 운영은 조용히
낡은 코드로 남고 이후 실행에서도 자가 복구되지 않는다. (doctypes.py 누락 — 이슈 #4)

Dockerfile 이 진실의 출처이고, deploy.sh 패턴은 그것을 빠짐없이 덮어야 한다.
"""
